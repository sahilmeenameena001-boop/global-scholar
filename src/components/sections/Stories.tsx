...
import { PortraitArt } from "../ui/PortraitArt";
import { DemoBadge } from "../ui/SectionHeading";

/** Illustrated stand-in for a person who does not exist — see `PortraitArt`. */
const meta = navByHref["/stories"];

export function Avatar({ name, colour, size = "size-14" }: { name: string; colour: string; size?: string }) {
  return <PortraitArt name={name} colour={colour} className={`${size} ring-1 ring-white/15`} decorative />;
}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        <Avatar name={s.name} colour={s.colour} />
        <div>
      ...
      <svg aria-hidden viewBox="0 0 300 60" className="mt-5 w-full">
        ...
        <text x="20" y="58" fontSize="9" fill="#a9b8d4">{s.city.split(",")[0]}</text>
        <text x="280" y="58" fontSize="9" fill="#a9b8d4" textAnchor="end">{s.destination.split(",")[0]}</text>
...
export function Stories() {
  return (
    <section id="stories" aria-labelledby="stories-title" data-chapter={5} className="relative scroll-mt-24 py-28 md:py-40">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <ChapterHead id="stories-title" index={meta.index} eyebrow={meta.eyebrow} title="Journeys that started with one honest conversation." />
